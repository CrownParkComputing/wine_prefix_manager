...
class ProcessService {
  ...
  ProcessService({CompressedGameService? compressedGameService, required PowerManagementService powerManagementService}) 
    ...
    try {
      if (exe.isCompressed && _compressedGameService != null) {
        try {
          ...
          final extractedExePath = await _compressedGameService!.extractGameForLaunch(exe);
          ...
          ProcessExitCallback wrappedOnProcessExit = (exePath, exitCode, errors) {
      ...
      if (prefix.type == PrefixType.wine) {
        ...
        final buildPath = prefix.wineBuildPath; // Removed unnecessary '!'
         ...
         if (!await protonDir.exists()) {
           ...
           onProcessExit(actualExe.path, -1, ['ERROR: Proton build directory does not exist: $resolvedPath']);
           return null;
         }
        ...
        if (wineExecutablePath.isEmpty) {
          onProcessExit(actualExe.path, -1, ['ERROR: Could not find wine executable in Proton build: $resolvedPath']);
          return null;
        }
        ...
        if (isMsi) {
          ...
          if (!await File(wineExecutablePath).exists()) {
              ...
              onProcessExit(actualExe.path, -1, ['ERROR: Could not find wine executable within Proton build at expected path: $wineExecutablePath']);
              return null;
          }
          ...
          if (command.isEmpty) {
            ...
            if (await File(wineExecutablePath).exists()) {
              ...
              onProcessExit(actualExe.path, 0, ['WARNING: Proton script not found, falling back to wine directly']);
            } else {
        ...
        if (!await File(command).exists()) {
           ...
           onProcessExit(actualExe.path, -1, ['ERROR: Wine executable not found: $command']);
           return null;
        }
      ...
      if (actualExe.launchOptions != null && actualExe.launchOptions!.trim().isNotEmpty) {
