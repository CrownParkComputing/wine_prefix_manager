...
class ExeEntry {
  ...
  final bool isCompressed; // True if this is a compressed game
  final String? compressedArchivePath; // Path to the compressed archive file
  final String? extractedBasePath; // Base path where game should be extracted
  final List<String> saveDataPaths; // Additional save data paths to backup during recompression
  final DateTime? lastExtracted; // When the game was last extracted
  final bool needsRecompression; // Flag to indicate files have been modified
  ...
  const ExeEntry({
    ...
    this.lastPlayed,
    // Compressed game fields
    this.isCompressed = false,
    this.compressedArchivePath,
    this.extractedBasePath,
    this.saveDataPaths = const [],
    this.lastExtracted,
    this.needsRecompression = false,
  });
  ...
  Map<String, dynamic> toJson() => {
    ...
    'compressedArchivePath': compressedArchivePath,
    'extractedBasePath': extractedBasePath,
    'saveDataPaths': saveDataPaths,
    'lastExtracted': lastExtracted?.toIso8601String(),
    'needsRecompression': needsRecompression,
  };
  ...
  factory ExeEntry.fromJson(Map<String, dynamic> json) {
     ...
     return ExeEntry(
        ...
        compressedArchivePath: json['compressedArchivePath'],
        extractedBasePath: json['extractedBasePath'],
        saveDataPaths: json['saveDataPaths'] != null
            ? List<String>.from(json['saveDataPaths'])
            : [],
        lastExtracted: json['lastExtracted'] != null
            ? DateTime.parse(json['lastExtracted'])
            : null,
        needsRecompression: json['needsRecompression'] ?? false,
     );
  }
  ...
  ExeEntry copyWith({
    ...
    DateTime? lastPlayed,
    bool? isCompressed,
    String? compressedArchivePath,
    String? extractedBasePath,
    List<String>? saveDataPaths,
    ...
    return ExeEntry(
      ...
      compressedArchivePath: compressedArchivePath ?? this.compressedArchivePath,
      extractedBasePath: extractedBasePath ?? this.extractedBasePath,
      saveDataPaths: saveDataPaths ?? this.saveDataPaths,
      lastExtracted: lastExtracted ?? this.lastExtracted,
      needsRecompression: needsRecompression ?? this.needsRecompression,
    );
  }
}
